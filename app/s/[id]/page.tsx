...
export default function PublicSurveyPage({ params }: { params: { id: string } }) {
  ...
  async function handleSubmit() {
    ...
    setSubmitting(false)
    if (error) { alert('Error submitting. Please try again.'); return }
    setSubmitted(true)
  }
        ...
        {questions.map((q, idx) => (
          <div key={q.id} style={{background:'white',borderRadius:12,padding:24,marginBottom:16,border:`1.5px solid ${errors[q.id]?'#EF4444':'#E4E7EE'}`,boxShadow:'0 2px 8px rgba(0,0,0,0.04)'}}>
            ...
            {q.type==='rating' && <>
              ...
              <div style={{display:'flex',justifyContent:'space-between',marginTop:8,fontSize:12,color:'#7C8494'}}>
            ...
            {q.type==='single' && q.choices.map((c, i) => (
              <label key={i} onClick={() => setAnswer(q.id, c)}
                style={{display:'flex',alignItems:'center',gap:12,padding:'12px 16px',border:'1.5px solid',borderColor:answers[q.id]===c?accentColor:'#E4E7EE',borderRadius:10,cursor:'pointer',marginBottom:8,background:answers[q.id]===c?`${accentColor}08`:'white',transition:'all 0.15s'}}>
            ...
            {q.type==='multiple' && q.choices.map((c, i) => {
              const selected = (answers[q.id] || []).includes(c)
              return (
                <label key={i} onClick={() => toggleMultiChoice(q.id, c)}
                  style={{display:'flex',alignItems:'center',gap:12,padding:'12px 16px',border:'1.5px solid',borderColor:selected?accentColor:'#E4E7EE',borderRadius:10,cursor:'pointer',marginBottom:8,background:selected?`${accentColor}08`:'white',transition:'all 0.15s'}}>
                  <div style={{width:18,height:18,borderRadius:4,border:`2px solid ${selected?accentColor:'#C7D2FE'}`,background:selected?accentColor:'white',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
            ...
            {q.type==='text' && (
              <input type="text" placeholder="Type your answer here..." value={answers[q.id] || ''}
            ...
            {q.type==='longtext' && (
              <textarea placeholder="Type your answer here..." value={answers[q.id] || ''} rows={4}
                onChange={e => setAnswer(q.id, e.target.value)}
